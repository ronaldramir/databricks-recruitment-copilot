...
- **Cómo quedó armado:** `mcp_server/vector_search.py` consulta el índice `recruitment_copilot.silver.silver_resumes_index` vía `VectorSearchClient`, expuesto como tool `find_matching_resumes(job_description, limit)` solo en el MCP server (para el agente). Se probó exponerlo también como caja de búsqueda en el dashboard (`/api/match_resumes` + `dashboard/vector_search.py`), pero se revirtió (2026-08-19): duplicaba la tool del agente como una segunda UI de búsqueda, diluyendo el patrón `day-3` de "el agente actúa, el dashboard solo observa". El dashboard quedó limitado a categorías (contexto) y shortlist (lo que el agente ya decidió).
